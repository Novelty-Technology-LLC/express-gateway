...
module.exports = function (params, config) {
  ...
  const httpProxyOptions = Object.assign(params, proxyOptions);
  ...
  return function proxyHandler(req, res) {
    const egContext = req.egContext;
    const target = balancer.nextTarget();
    const headers = Object.assign(
      { 'eg-request-id': egContext.requestID },
    ...
    if (isWebSocket) {
      logger.info('Proxying WebSocket connection to', target);

      proxy.ws(egContext.req, egContext.req.socket, egContext.req.headers, {
        target,
        ws: true,
        headers: egContext.req.headers,
        buffer: egContext.requestStream
      });
    } else {
      proxy.web(req, res, {
        target,
